  ...
  if [[ $TRAVIS_OS_NAME == 'linux' && $MPI != 'none' ]]; then 
    ...
    if [[ -z "$(ls -A ${DEPS_DIR}/boost)" ]]; then
      ...
      if [[ $COMPILER == 'clang++' ]]; then 
        ...
        ./bootstrap.sh --prefix=${DEPS_DIR}/boost/ --with-libraries=serialization,mpi,thread,date_time,system,iostreams,timer,filesystem,program_options,numpy,numeric,python --with-toolset=clang
        ./b2 clean
