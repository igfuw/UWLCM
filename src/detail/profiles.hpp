...
namespace detail
{
  ...
  struct profiles_t
  {
    setup::arr_1D_t th_e, p_e, rv_e, rl_e, th_ref, rhod, w_LS, hgt_fctr, mix_len, relax_th_rv_coeff;
    ...
    // TODO: should have a halo both up and down, not only up like now; then it should be interpolated in courant calculation
      th_e(nz), p_e(nz), rv_e(nz), rl_e(nz), th_ref(nz), rhod(nz+1), w_LS(nz), hgt_fctr(nz), mix_len(nz), relax_th_rv_coeff(nz)
    {
      geostr[0].resize(nz);
      geostr[1].resize(nz);
  ...
  {
    setup::arr_1D_t *th_e, *p_e, *rv_e, *rl_e, *th_ref, *rhod, *w_LS, *hgt_fctr, *geostr[2], *mix_len, *relax_th_rv_coeff;
  };
