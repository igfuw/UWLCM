  ...
  if(params.buoyancy_wet)
    F(ijk).reindex(this->zero) = 
      (libcloudphxx::common::earth::g<setup::real_t>() / si::metres_per_second_squared) * (
        (th(ijk).reindex(this->zero) - (*params.profs.th_e)(this->vert_idx)) / (*params.profs.th_reference)(this->vert_idx)
        + eps * (rv(ijk).reindex(this->zero) - (*params.profs.rv_e)(this->vert_idx)) 
        - (r_l(ijk).reindex(this->zero) - (*params.profs.rl_e)(this->vert_idx))
      );
  else
    F(ijk).reindex(this->zero) = 
      (libcloudphxx::common::earth::g<setup::real_t>() / si::metres_per_second_squared) * (
