...
void copy_profiles(setup::arr_1D_t &th_e, setup::arr_1D_t &T_e, setup::arr_1D_t &rv_e, setup::arr_1D_t &th_ref, setup::arr_1D_t &pre_ref, setup::arr_1D_t &rhod, setup::arr_1D_t &w_LS, setup::arr_1D_t &hgt_v, setup::arr_1D_t &hgt_s, params_t &p)
{
  ...
  p.T_e = new setup::arr_1D_t(T_e.dataFirst(), T_e.shape(), blitz::neverDeleteData);
...
void run(int nx, int nz, const user_params_t &user_params)
{
  ...
  setup::arr_1D_t th_e(nz), T_e(nz), rv_e(nz), th_ref(nz), pre_ref(nz), rhod(nz+1), w_LS(nz), hgt_fctr_vctr(nz), hgt_fctr_sclr(nz); 
  ...
  // assign their values
  case_ptr->env_prof(th_e, T_e, rv_e, th_ref, pre_ref, rhod, w_LS, hgt_fctr_vctr, hgt_fctr_sclr, nz, user_params);
  // pass them to rt_params
  copy_profiles(th_e, T_e, rv_e, th_ref, pre_ref, rhod, w_LS, hgt_fctr_vctr, hgt_fctr_sclr, p);
