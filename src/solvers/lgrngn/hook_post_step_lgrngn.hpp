...
void slvr_lgrngn<ct_params_t>::hook_post_step()
{
  ...
  if (this->rank == 0)
  {
    if((this->timestep >= params.user_params.precoal_start) && (int(this->timestep - params.user_params.precoal_start) % int(params.user_params.precoal_outfreq) == 0))
    ;
      prtcls->store_ijk(this->timestep * params.user_params.dt);
  }
  this->mem->barrier();

  if(params.user_params.precoal_start >= 0 && this->timestep == params.user_params.nt) // save precoal distance at the end of the simulation
  {
