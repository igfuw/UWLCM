...
void slvr_lgrngn<ct_params_t>::update_rhs(
    libmpdataxx::arrvec_t<typename parent_t::arr_t> &rhs,
    const typename parent_t::real_t &dt,
    const int &at
  )
{
  negtozero(this->mem->advectee(ix::rv)(this->ijk), "rv at start of lgrngn update_rhs");
  parent_t::update_rhs(rhs, dt, at); //includes the change from theta perturbation to regular theta

  if(at==0)
  {
    rv_pre_cond(this->ijk) = this->state(ix::rv)(this->ijk); 
    th_pre_cond(this->ijk) = this->full_th(this->ijk); 
