#pragma once
#include "blk_1m_ice/slvr_blk_1m_ice_common.hpp"
     ...
     if(at == 0)
     {
       ...
       for (int i = this->i.first(); i <= this->i.last(); ++i)
         for (int j = this->j.first(); j <= this->j.last(); ++j)
         {
           ...
           iceA_precipitation_rate_arg = this->iceA_precipitation_rate(i, j, this->k),
           iceB_precipitation_rate_arg = this->iceB_precipitation_rate(i, j, this->k);
           const auto
           rhod   = (*this->mem->G)(i, j, this->k),
           rr     = this->state(parent_t::ix::rr)(i, j, this->k),
           ria     = this->state(parent_t::ix::ria)(i, j, this->k),
           rib     = this->state(parent_t::ix::rib)(i, j, this->k);
           this->liquid_puddle += - libcloudphxx::blk_1m::rhs_columnwise<real_t>(
             this->params.cloudph_opts, precipitation_rate_arg, rhod, rr, this->params.dz
           );
           // TODO: ice puddle
