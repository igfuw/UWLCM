...
// calc upward radiative flux through the bottom of the cells
{
  ...
  for(int z = 0 ; z < nz; ++z)
  {
    ...
    if(z==0)
      F(i, z) += setup::F_0 * exp(- (nz - z - 1) * params.dz * sum); 
    else
      F(i, z) += setup::F_0 * exp(- (nz - z - 0.5) * params.dz * sum); 

    if(z > 0)
    {
      sum = blitz::sum(r_l(i, blitz::Range(0, z-1)), blitz::tensor::j);
      F(i, z) += setup::F_1 * exp(- (z - 0.5) * params.dz * sum);
    }

    F(i, z) += where(z > k_i(i), 
      setup::c_p * setup::rho_i * setup::D *
      (0.25 * pow((z - 0.5) * params.dz - (k_i(i) - .5) * params.dz, 4./3) +
      (k_i(i) - .5) * params.dz * pow((z - 0.5) * params.dz - (k_i(i) - .5) * params.dz, 1./3))
      , 0);
  }
// smoothing
  tmp1(ijk)=F(ijk);
...
void slvr_lgrngn<ct_params_t>::subsidence(const int &type) // large-scale vertical wind
{
  ...
  this->xchng_sclr(tmp1, i, j);
  F(i, j).reindex({0,0}) = - (*params.w_LS)(blitz::tensor::j) * (tmp1(i, j + 1).reindex({0,0})- tmp1(i, j - 1).reindex({0,0})) / (2. * params.dz); 
// smoothing
  tmp1(ijk)=F(ijk);
  this->xchng_sclr(tmp1, i, j); 
  F(i, j) = 0.25 * (tmp1(i, j + 1) + 2 * tmp1(i, j) + tmp1(i, j - 1));
