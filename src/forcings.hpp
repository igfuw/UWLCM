...
// calc upward radiative flux through the bottom of the cells
{
  ...
  k_i(this->hrzntl_subdomain) = blitz::first( tmp1(ijk).reindex(this->zero) < params.ForceParameters.q_i, this->vert_idx); 
  ...
  F(noground) = params.ForceParameters.F_0 * exp(- (nz - this->vert_idx - 0.5) * params.dz * tmp1(noground)); 
  F(ground) = params.ForceParameters.F_0 * exp(- (nz - this->vert_idx - 1) * params.dz * tmp1(ground));
  ...
  for(int z = 1 ; z < nz-1; ++z)
    tmp1(idxperm::pi<perm_no>(z, this->hrzntl_subdomain)) += tmp1(idxperm::pi<perm_no>(z-1, this->hrzntl_subdomain));
  F(noground) += params.ForceParameters.F_1 * exp(- (this->vert_idx - 0.5) * params.dz * tmp1(notop));
